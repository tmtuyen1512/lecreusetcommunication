...
public class ItemInfoInsert extends DoInsertBase  implements Serializable {


    public ItemInfoInsert() {
        fileName = ConstComm.RDIVarekartotek_Filename;
        tableName = DBUtil.DBItemInfo.class.getSimpleName();
        FIX_RECORD_LENGTH = 60;
    }
    ...
    public void addDBItem(SQLiteDatabase db, File f){
        try {
            ...
            for(int i = 0; i< arr.size(); i++) {
                ...
                doInsertDBItemInfo(db, i, arr.get(i), 0, 99);
    ...
    private void doInsertDBItemInfo(SQLiteDatabase db, int index, ArrayList<char[]> arr, int from, int to) {
        if (to < arr.size() - 1) {
            doInsertDBItemInfo(db, index, arr, to + 1, to + 100);
        }
        if(to > arr.size() - 1)
            to = arr.size() - 1;
        ...
        SQLiteStatement stmt = db.compileStatement(
                "Insert into "+tableName+"(itemNumber, itemDescription, itemPrice) VALUES "
        ...
        for (int i = from; i <= to; i++) {
            ...
            if(s.length == FIX_RECORD_LENGTH) {
                ...
                bindValue(stmt, k + 2, s, 20, 30);
                bindValue(stmt, k + 3, s, 50, 10);
