...
public class PartnerItemReferenceInsert extends DoInsertBase implements Serializable {

    public PartnerItemReferenceInsert() {
        fileName = ConstComm.PartnerVareReference_Filename;
        tableName = DBUtil.DBPartnerItemReference.class.getSimpleName();
    ...
    public void addDBItem(SQLiteDatabase db, File f) {
        try {
            ...
            for(int i = 0; i< arr.size(); i++) {
                ...
                doDBPartnerItemReference(db, i, arr.get(i), 0, 99);
    ...
    private void doDBPartnerItemReference(SQLiteDatabase db, int index, ArrayList<char[]> arr, int from, int to) {
        if (to < arr.size() - 1) {
            doDBPartnerItemReference(db, index, arr, to + 1, to + 100);
        }
        ...
        for (int i = from; i <= to; i++) {
            if(arr.get(i).length == FIX_RECORD_LENGTH) {
                ...
                errorLines.add(0, index * FileUtil.BLOCK + i + 1);
            }
        }
        if(valuesBuilder.length() == 0){
            errorLines.add(0);
