...
public class ItemGroupInsert extends DoInsertBase implements Serializable {


    public ItemGroupInsert() {
        fileName = ConstComm.Varegruppe_Filename;
        tableName = DBUtil.DBItemGroup.class.getSimpleName();
    ...
    public void addDBItem(SQLiteDatabase db, File f){
        try {
            ...
            for(int i = 0; i< arr.size(); i++) {
                ...
                doInsertDBItemGroup(db, i, arr.get(i), 0, 99);
    ...
    private void doInsertDBItemGroup(SQLiteDatabase db, int index, ArrayList<char[]> arr, int from, int to) {
        if (to < arr.size() - 1) {
            doInsertDBItemGroup(db, index, arr, to + 1, to + 100);
        }
        if(to > arr.size() - 1)
            to = arr.size() - 1;
        ...
        SQLiteStatement stmt = db.compileStatement(
                "Insert into "+tableName+"(itemGroup, itemGroupLabel) VALUES "
